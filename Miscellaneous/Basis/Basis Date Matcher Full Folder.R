...
fullSet = data.frame()
dates = read_excel("Miscellaneous/Basis/Dates.xlsx")

cropType = dlg_message(message = "Yes for Corn, No for Soybeans", type = "yesno")[["res"]]

if (cropType == "yes") {
  files = list.files("Miscellaneous/Basis/CornData")
} else {
  files = list.files("Miscellaneous/Basis/SoybeanData")
}
...
for (filename in files) {
  ...
  if (cropType == "yes") {
    basis = read_csv(paste("Miscellaneous/Basis/CornData/", filename, sep = ""), skip = 1)
  } else {
    basis = read_csv(paste("Miscellaneous/Basis/SoybeanData/", filename, sep = ""), skip = 1)
  }
...
if (dlg_message(message = "Export data to a CSV?", type = "yesno")[["res"]] == "yes") {
  ...
  write.csv(fullSet, paste("Miscellaneous/Basis/", filename, ".csv", sep = ""), row.names = FALSE)
}
