...
# Load files
KStateFile = read.csv(choose.files(default = paste0(getwd(), "/Miscellaneous/Basis/*.csv"), caption = "Select Crop File:",
             multi = FALSE, filters = Filters,
             index = nrow(Filters)))

citiesCounties = read_excel("Miscellaneous/Basis/Cities and Counties.xlsx")
...
if (dlg_message(message = "Export kLocBasisMerge to a RDS?", type = "yesno")[["res"]] == "yes") {
  ...
  saveRDS(kLocBasisMerge, paste("Miscellaneous/Basis/", filename1, ".rds", sep = ""))
}
...
if (dlg_message(message = "Export weeklyAverageList to a RDS?", type = "yesno")[["res"]] == "yes") {
  ...
  saveRDS(weeklyAverageList, paste("Miscellaneous/Basis/", filename1, ".rds", sep = ""))
}
