...
# Returns True for first day of post harvest
isHarvestDate = function(marketDay, harvestDay) {
  if (marketDay == harvestDay) {
    return(TRUE)
  } else {
    return(FALSE)
  }
...
isActualizedPO = function(cropYear, cropYear1, cropYear2, futuresMarket, MY) {
  ...
  harvest = mdy(futuresMarket$Date[which(mdy(futuresMarket$Date) %within% intervalPost)[1]])
  ...
  if (is.null(cropYear1) || !is.null(cropYear1)) {
    for (row in 1:nrow(marketingYear)) {
      ...
      # Check if first day of post harvest
      else if (isHarvestDate(marketingYear$Date[row], harvest)) {
        if (totalSold < 50) {
          percentSold = 50 - totalSold
          totalSold = totalSold + percentSold
