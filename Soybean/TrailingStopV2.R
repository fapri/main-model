...
# V2: price must be below the new percentiles in order to trigger a TS sale
...
# Percentile Drops
fivePercentDrop = function(marketingYear, row, previousDayPercentile, currentDayPercentile){
...
trailingStopTrigger = function(cropYear, featuresObject) {
  ...
  for(row in 2:nrow(marketingYear)) {
    ...
    else if(fivePercentDrop(marketingYear, row, marketingYear$Percentile[row - 1], marketingYear$Percentile[row])) {
