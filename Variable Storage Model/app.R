  ...
  navbarPage("Marketing Strategies",
             tabPanel("Home",
                      fluidPage(
                        ...
                        fluidRow(column(12,
                                        sliderInput("availableFarmStorage", "On-Farm Storage Capacity:", min = 0, max = 100, value = 50, step = 10)
                        )
                        )
                      )
             ),
...
server <- shinyServer(function(input,output,session){
  ...
  source('Storage_testapp.R')
  
  AvailFarmStorage <- reactive({
    PercentageCanStore = input$availableFarmStorage
  })
