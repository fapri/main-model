...
isActualized = function(cropYear){
  ...
  dec31NC = paste("12-31", toString(year(mdy(cropYear$`Start Date`))), sep="-")
  interval1 = interval(mdy(jan1NC), mdy(may31NC))
  interval2 = interval(mdy(june1NC), mdy(dec31NC))
        ...
        if(nrow(priceObjectiveActualized) == 0 || difftime(triggers$Date[tRow], priceObjectiveActualized$Date[nrow(priceObjectiveActualized)]) >= 7){
          ...
          if(percentSold < 50){
            ...
            if(dim(priceObjectiveActualized)[1] != 0){
              #check if trigger date is in a restricted interval
              ...
              #checks the second interval. The first half of interval2 contains NC dates.
              #this interval is separate from interval1 because at June 1, the percentile rule resets.
              else if(triggers$Date[tRow] %within% interval2){
                tempRows = NA
                #create a list to get the actualized sales rows within an interval. This will be used to ensure 1 sale per percentils
                tempRows = which(priceObjectiveActualized$Date %within% interval2)
                #check if a sale was made in that percentile
                if(!(triggers$Percentile[tRow] %in% priceObjectiveActualized$Percentile[tempRows])){
                  #PO, ATH, TDH at 10% increments
                  percentSold = percentSold + 10
              ...
              #if trigger date isnt in a restricted interval we can just make the sale
              else{
                #PO, ATH, TDH at 10% increments
                percentSold = percentSold + 10
                priceObjectiveActualized = rbind(priceObjectiveActualized, data.frame("Date" = triggers$Date[tRow], 
          ...
          if(percentSold < 100){
            ...
            if(difftime(triggers$Date[tRow], priceObjectiveActualized$Date[nrow(priceObjectiveActualized)]) >= 7){
              ...
              if(percentSold <= 90){
                ...
                if(triggers$Date[tRow] %within% interval2){
                  tempRows = NA
                  #create a list to get the actualized sales rows within an interval. This will be used to ensure 1 sale per percentils
                  tempRows = which(priceObjectiveActualized$Date %within% interval2)
                  #check if a sale was made in that percentile
                ...
                #if the date isnt in restricted interval, we can just make the sale
                else{
                  #PO, ATH, TDH at 10% increments
                  percentSold = percentSold + 10
                  priceObjectiveActualized = rbind(priceObjectiveActualized, data.frame("Date" = triggers$Date[tRow], 
