...
createCropYear = function(cropYear, startDate, stopDate, type) {
  ...
  if (type == "soybean") {
    ...
    dateCol = which(colnames(futuresMarket) == "Date")
    nearbyCol = which(colnames(futuresMarket) == "NearbyOC")
    NCCol = which(colnames(futuresMarket) == "NovNC")
    NC1yrCol = which(colnames(futuresMarket) == "NovNC1yr")
    NC2yrCol = which(colnames(futuresMarket) == "NovNC2yr")
  ...
  if (type == "corn") {
    ...
    dateCol = which(colnames(futuresMarket) == "Date")
    nearbyCol = which(colnames(futuresMarket) == "NearbyOC")
    NCCol = which(colnames(futuresMarket) == "DecNC")
    NC1yrCol = which(colnames(futuresMarket) == "DecNC1yr")
    NC2yrCol = which(colnames(futuresMarket) == "DecNC2yr")
    MarNCCol = which(colnames(futuresMarket) == "DecNC")
  }
  
  
  
  marketingYearPre = futuresMarket[which(mdy(futuresMarket$Date) %within% intervalPre), c(dateCol, NCCol, MarNCCol)]
  marketingYearPre = setNames(marketingYearPre, c("Date", "Price", "MarNCPrice"))
  marketingYearPost = futuresMarket[which(mdy(futuresMarket$Date) %within% intervalPost), c(dateCol, nearbyCol, MarNCCol)]
  marketingYearPost = setNames(marketingYearPost, c("Date", "Price", "MarNCPrice"))
  marketingYear = rbind(marketingYearPre, marketingYearPost)
  

  marketingYearMY = futuresMarket[which(mdy(futuresMarket$Date) %within% intervalPrePost), c(dateCol, NCCol)]
  marketingYearMY = setNames(marketingYearMY, c("Date", "NC Price"))
  
  marketingYear[,c("Baseline", "60th", "70th", "80th", "90th", "95th", "Basis", "Percentile", "MarPercentile")] = NA
