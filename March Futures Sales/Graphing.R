...
#Set Color Palette
myColors = c("#ffff00", # Price objective
             "#ffff00", # Price objective special
             "#ffff00", # Price objective March
             "#ffff00", # Price objective march special
             "#800080", # Trailing Stop
             "#800080", # Trailing Stop special
             "#800080", # Trailing Stop march
             "#800080", # Trailing Stop march special
             "#ed7d31", # Seasonal
             "#4286f4", # Ten day high
             "#ff4242", # All time high
             "#FF69B4", # EYTS
             "#000080") # Multi-year

colorLabels = c("Price Objective", 
                "Price Objective Special",
                "Price Objective March", 
                "Price Objective March Special",
                "Trailing Stop", 
                "Trailing Stop Special", 
                "Trailing Stop March", 
                "Trailing Stop March Special", 
                "Seasonal", 
                "Ten Day High", 
                "All Time High", 
                "End of Year Trailing Stop", 
                "Multi-Year")
...
if (type == "soybean") {
  for (i in 1:length(Soybean_CropYearObjects)) {
    if (nrow(Soybean_CropYearObjects[[i]][['PO Actualized']]) > 0) {
      ...
      POMYTitle = "Price Objective with Multi-Year"
      names(Soybean_CropYearObjects[[i]]$`Marketing Year MY`) = names(Soybean_CropYearObjects[[i]]$`Marketing Year`[-c(3,12)])
